...
def get_user_preferences():
    ...
    # Get icon path
    while True:
        icon_path = input("\n🖼️ Program icon path (Press Enter to skip this option): ").strip()
        if not icon_path:
            icon_path = None
            break
...
def install_requirements():
    ...
    try:  # استخدام إصدار أقدم لتوافق أفضل
...
def protect_source():
    ...
    try:
        ...
        print(f"⚠️ Warning: Code encryption failed. Reason: {str(e)}")
        print("⚠️ Continuing without additional encryption...")
        return False
